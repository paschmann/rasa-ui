...
function TrainingController($scope, $rootScope, $interval, $http, Rasa_Status, Agent, AgentRegex, ExpressionParameters, IntentExpressions, yaml, AgentEntities, AgentActions, AgentSynonyms, SynonymsVariants, appConfig) {
  ...
  $scope.trainUsingRawData = function () {
    ...
    $http.post(appConfig.api_endpoint_v2 + "/rasa/model/train?agent_name=" + agentToTrain.agent_name + "&agent_id=" + agentToTrain.agent_id + "&comment=" + $scope.comment, JSON.parse($scope.raw_data)).then(
      function (response) {
  ...
  $scope.saveRawDataToFile = function () {
    let data = new Blob([JSON.stringify($scope.raw_data, null, 2)], {
      type: 'text/plain'
    });
