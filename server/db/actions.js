...
function getBotActionsAndResponses(req, res, next) {
  ...
  db.all('select * from actions where bot_id = ? order by action_id desc', req.query.bot_id, function(err, actions) {
    if (err) {
      logger.winston.error('Error in getBotActionsAndResponses:', err);
    } else {
      var actionIds = [];
      ...
      if (actionIds.length > 0) {
        ...
        var placeholders = actionIds.map(() => '?').join(',');
        db.all('select * from responses where action_id in (' + placeholders + ')  order by action_id desc', actionIds, function(err, responses) {
          if (err) {
            logger.winston.error('Error fetching responses:', err);
          } else {
